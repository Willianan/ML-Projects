...
class LR(model):
	def __init__(self, input_dim=None, output_dim=1, init_path=None,
	             opt_algo='gd', learning_rate=1e-2, l2_weight=0,
	             random_seed=None):
		model.__init__(self)
		...
		with self.graph.as_default():
			if random_seed is not None:
				...
				logits = logits)) + l2_weight * tf.nn.l2_loss(xw)
